...
export default function LeadProfile({ lead, currentAdmin, onClose, onUpdate }) {
  ...
  const waMsg = encodeURIComponent(`Hola ${lead.nombre}, soy asesor de Financial Boost. Quería dar seguimiento a tu solicitud de ${lead.monto_necesario}. ¿Tienes un momento?`)
        ...
        <div className="bg-white w-full md:max-w-2xl h-full overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
          ...
          <div className="bg-primary px-6 py-4 flex items-center justify-between sticky top-0 z-10">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="text-xs text-primary-300">#{lead.id?.slice(0,8).toUpperCase()}</span>
                <span className="text-xs text-primary-300">·</span>
                <span className="text-xs text-primary-200">{lead.email}</span>
              </div>
            </div>
            <button onClick={onClose} className="text-white hover:text-primary-200"><X size={22}/></button>
          </div>
          ...
          <div className="p-4 sm:p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <a href={`tel:${lead.telefono}`} className="flex-1 bg-secondary hover:bg-secondary-700 text-white font-semibold py-2.5 rounded-xl flex items-center justify-center gap-2 text-sm transition-colors">
            ...
            <div>
              ...
              {existingLoan && <p className="text-xs text-secondary-600 mb-3 bg-secondary-50 rounded-lg px-3 py-2">Editando préstamo activo · Al guardar cambios se actualizará para el cliente.</p>}
              {!existingLoan && <p className="text-xs text-secondary-600 mb-3 bg-secondary-50 rounded-lg px-3 py-2">Al guardar cambios se guardan estos valores en el lead. El préstamo real solo se crea con "Crear prestamo".</p>}
            ...
            <div className="space-y-3 pb-4">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={save} disabled={saving} className="flex-1 bg-primary hover:bg-primary-950 disabled:opacity-60 text-white font-bold py-3 rounded-xl transition-all">
